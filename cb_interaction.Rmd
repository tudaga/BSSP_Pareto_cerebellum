...
finalExpression <- ggarrange(grm1, calb2, plcb4, plcb1)


tsne <- TSNEPlot(ubc_s, label = T)

fp <- FeaturePlot(ubc_s, c('Grm1', 'Calb2', 'Plcb4', 'Plcb1'), cols = c('yellow', 'purple'))
# Plot and group by cerebellar region 
dimplot = DimPlot(ubc_s, reduction = 'tsne', group.by = 'region')
...
SCEDotPlot = logNormCounts(ubc_sce)
SCEDotPlot$ident = factor(SCEDotPlot$ident, levels = c('Brinp2_On_UBCs', 'Intermediate_UBCs', 'Calb2_Off_UBCs'), ordered = T)
SCEDotPlot <- plotDots(SCEDotPlot, group = "ident", features = c('Grm1', 'Plcb4', 'Calb2', 'Plcb1'),
max_ave = 1, low_color = "white", high_color = "blue")
